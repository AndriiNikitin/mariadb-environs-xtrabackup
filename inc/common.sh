...
#        cd $MYSQL_BASEDIR
        ...
        if [[ ${CLIENT_VERSION_STRING} =~ Ver.*5\.7\.[0-9] ]] ; then
            ...
            --initialize-insecure"
        elif [ ! -z ${MYSQL_SRCDIR} ] ; then
            INSTALL_CMD="bash ${MYSQL_INSTALL_DB} \
            --defaults-file=${MYSQLD_VARDIR}/my.cnf \
            --builddir=${MYSQL_BLDDIR:-$MYSQL_SRCDIR} \
            --srcdir=${MYSQL_SRCDIR}" 
        else
...
EOF
        [ ! -z "$MYSQL_EXTRA_CNF" ] && echo "!include $MYSQL_EXTRA_CNF" >> ${MYSQLD_VARDIR}/my.cnf

#        if [ ! -z "$MYSQLD_DATADIR" ] && [ -f "$MYSQLD_DATADIR"/last_MYSQLD_EXTRA_MY_CNF_OPTS ]
#        then
#           local last_MYSQLD_EXTRA_MY_CNF_OPTS=$(cat "$MYSQLD_DATADIR"/last_MYSQLD_EXTRA_MY_CNF_OPTS 2>/dev/null)
#           if [ "$(trim $last_MYSQLD_EXTRA_MY_CNF_OPTS)" != "$(trim ${MYSQLD_EXTRA_MY_CNF_OPTS:-})" ] ; then
#              # options changed - need rebuild datadir
#              (killall mysqld && sleep 3) || true
...
#    echo "${MYSQLD_EXTRA_MY_CNF_OPTS:-}" > "$MYSQLD_DATADIR"/last_MYSQLD_EXTRA_MY_CNF_OPTS
